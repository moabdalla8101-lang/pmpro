...
async function main() {
  const jsonPath = '/Users/mohamed/Downloads/pmp/pmp_questions_ultimate/extracted_questions_stakeholders1.json';
  ...
  // Get Stakeholder Management knowledge area ID
  const kaResult = await pool.query(
    "SELECT id FROM knowledge_areas WHERE name = 'Project Stakeholder Management' LIMIT 1"
  );
  
  if (kaResult.rows.length === 0) {
    console.error('Error: Project Stakeholder Management knowledge area not found in database');
    process.exit(1);
  }
  
  const stakeholderKnowledgeAreaId = kaResult.rows[0].id;
  console.log(`✅ Found Stakeholder Management knowledge area ID: ${stakeholderKnowledgeAreaId}`);
  ...
  for (let i = 0; i < validQuestions.length; i++) {
    ...
    try {
      ...
      const questionId = `STAKE-${q.number}`;
      ...
      skipped.push({
        ...
        preview: questionText ? questionText.substring(0, 100) + '...' : ''
      });
