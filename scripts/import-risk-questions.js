...
async function main() {
  const jsonPath = '/Users/mohamed/Downloads/pmp/pmp_extractions/extraction_20260116_181304_risk/extracted_questions.json';
  ...
  // Get Risk Management knowledge area ID
  const kaResult = await pool.query(
    "SELECT id FROM knowledge_areas WHERE name = 'Project Risk Management' LIMIT 1"
  );
  
  if (kaResult.rows.length === 0) {
    console.error('Error: Project Risk Management knowledge area not found in database');
    process.exit(1);
  }
  
  const riskKnowledgeAreaId = kaResult.rows[0].id;
  console.log(`✅ Found Risk Management knowledge area ID: ${riskKnowledgeAreaId}`);
  ...
  for (let i = 0; i < validQuestions.length; i++) {
    ...
    try {
      ...
      // Validate options exist (except for drag_and_match which might have drag_drop_pairs instead)
      ...
      // For drag_and_match, validate that we have either options or drag_drop_pairs
      if (questionType === 'drag_and_match' && (!options || options.length === 0) && (!dragDropPairs || dragDropPairs.length === 0)) {
        skipped.push({
          number: q.number,
          questionId: `RISK-${q.number}`,
          reason: 'No options or drag_drop_pairs found for drag-and-drop question',
          preview: data.question_text.substring(0, 100) + '...'
        });
        continue;
      }
      ...
      const questionId = `RISK-${q.number}`;
      ...
      if (questionType === 'drag_and_match') {
        ...
        let matches = {};
        if (data.correct_answer && data.correct_answer.pairs) {
          data.correct_answer.pairs.forEach(pair => {
        ...
        if ((!options || options.length === 0) && rightItems.length > 0) {
