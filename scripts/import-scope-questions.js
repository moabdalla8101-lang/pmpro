...
async function main() {
  const jsonPath = '/Users/mohamed/Downloads/pmp/pmp_extractions/extraction_20260120_221651_scope/extracted_questions.json';
  ...
  // Get Scope Management knowledge area ID
  const kaResult = await pool.query(
    "SELECT id FROM knowledge_areas WHERE name = 'Project Scope Management' LIMIT 1"
  );
  
  if (kaResult.rows.length === 0) {
    console.error('Error: Project Scope Management knowledge area not found in database');
    process.exit(1);
  }
  
  const scopeKnowledgeAreaId = kaResult.rows[0].id;
  console.log(`✅ Found Scope Management knowledge area ID: ${scopeKnowledgeAreaId}`);
  ...
  for (let i = 0; i < validQuestions.length; i++) {
    ...
    try {
      ...
      const questionId = `SCOPE-${q.number || i + 1}`;
      ...
      // Validation moved below after correct answer check
