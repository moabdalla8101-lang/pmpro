...
async function main() {
  const jsonPath = '/Users/mohamed/Downloads/pmp/pmp_extractions/extraction_20260120_221054_schedule/extracted_questions.json';
  ...
  // Get Schedule Management knowledge area ID
  const kaResult = await pool.query(
    "SELECT id FROM knowledge_areas WHERE name = 'Project Schedule Management' LIMIT 1"
  );
  
  if (kaResult.rows.length === 0) {
    console.error('Error: Project Schedule Management knowledge area not found in database');
    process.exit(1);
  }
  
  const scheduleKnowledgeAreaId = kaResult.rows[0].id;
  console.log(`✅ Found Schedule Management knowledge area ID: ${scheduleKnowledgeAreaId}`);
  ...
  for (let i = 0; i < validQuestions.length; i++) {
    ...
    try {
      ...
      const questionId = `SCHEDULE-${q.number || i + 1}`;
