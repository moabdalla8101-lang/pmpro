...
async function main() {
  const jsonPath = '/Users/mohamed/Downloads/pmp/pmp_extractions/extraction_20260120_220728_quality/extracted_questions.json';
  ...
  // Get Quality Management knowledge area ID
  const kaResult = await pool.query(
    "SELECT id FROM knowledge_areas WHERE name = 'Project Quality Management' LIMIT 1"
  );
  
  if (kaResult.rows.length === 0) {
    console.error('Error: Project Quality Management knowledge area not found in database');
    process.exit(1);
  }
  
  const qualityKnowledgeAreaId = kaResult.rows[0].id;
  console.log(`✅ Found Quality Management knowledge area ID: ${qualityKnowledgeAreaId}`);
  ...
  for (let i = 0; i < validQuestions.length; i++) {
    ...
    try {
      ...
      const questionId = `QUALITY-${q.number || i + 1}`;
